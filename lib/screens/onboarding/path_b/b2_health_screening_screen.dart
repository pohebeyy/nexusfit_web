...
/// B2: Health Screening - проверка здоровья для Path B
class B2HealthScreeningScreen extends StatefulWidget {
  const B2HealthScreeningScreen({super.key});

  @override
  State<B2HealthScreeningScreen> createState() => _B2HealthScreeningScreenState();
}

class _B2HealthScreeningScreenState extends State<B2HealthScreeningScreen> {
  final List<String> _selectedIssues = [];
  final TextEditingController _otherIssueController = TextEditingController(); // Контроллер для ввода
  bool _showOtherInput = false; // Флаг видимости инпута
  ...
  Widget _buildOtherInput() {
    return AnimatedContainer(
      ...
      child: TextField(
        ...
        onChanged: (value) {
          setState(() {}); // Обновляем кнопку при вводе
        },
      ),
    );
  }
